...
<script type="text/javascript" src="../includes/js/greybox/AJS.js"></script>
<script type="text/javascript" src="../includes/js/greybox/AJS_fx.js"></script>
<script type="text/javascript" src="../includes/js/greybox/gb_scripts.js"></script>

<link href="../includes/js/greybox/gb_styles.css" rel="stylesheet" type="text/css" />
		...
		<? foreach ($this->listings as $listing): ?>
			...
			?>
				<div class="listing_address">
					<h4><? echo $listing->{FIELDNAME_RESIDENTIAL_AREA}; ?></h4>
...
<? 
						...
						{
							echo '<a href="index.php?option=com_portaladv&view=details&listing=' . $listing->{FIELDNAME_RESIDENTIAL_LISTING_DATABASE_ID} . '"><img src="/rets_photos/image-' . $listing->{FIELDNAME_RESIDENTIAL_LISTING_DATABASE_ID} . '-1.jpg" width="180" /></a>';
						} 
						else
						{
...
?>
					...
					<div class="listing_info">
						<?php ## modified by Rich S Wyatt to strip the comma from the numeral ?>
						<span class="listing_price"><? echo '$' . number_format(str_replace(",","",$listing->{FIELDNAME_RESIDENTIAL_PRICE})); ?></span><br />
						<span class="listing_bedbath"><? echo number_format($listing->{FIELDNAME_RESIDENTIAL_NUM_BEDROOMS},0);  ?> <? echo BEDROOM_ABBREV_1; ?> / <? echo number_format($listing->{FIELDNAME_RESIDENTIAL_NUM_BATHROOMS},0); ?> <? echo BATHROOM_ABBREV_1; ?> / <? echo number_format($listing->{FIELDNAME_RESIDENTIAL_SQUAREFEET}); ?> <? echo SQUAREFEET_ABBREV; ?></span><br />
						<span class="listing_type"><? echo ucwords(strtolower(str_replace(","," / ",$listing->{FIELDNAME_LISTING_SUB_TYPE}))); ?></span>
						<?php 
						if ($listing->{FIELDNAME_RESIDENTIAL_PROP_TYPE} == 'Partial Ownership') {
...
?>
							...
							if ($listing->{FIELDNAME_LISTING_VIRTUAL_TOUR_LINK}) {
								echo "<input type=\"button\" class=\"portaladv_button\" onmouseover=\"this.className='portaladv_button_hover';\" onmouseout=\"this.className='portaladv_button';\" onclick=\"return GB_showFullScreen('Beaver Creek Online Virtual Tour','" . $listing->{FIELDNAME_LISTING_VIRTUAL_TOUR_LINK} . "');\" value=\"" . BUTTON_VIRTUAL_TOUR . "\"></input>";
							}		
							?>
						</strong>
						...
						<table>
							<tr>
								<td>
									...
									{
										echo '<img src="/rets_photos/image-' . $listing->{FIELDNAME_RESIDENTIAL_LISTING_DATABASE_ID} . '-2.jpg" width="50" />';
									}
									?>
								</td>
								<td>
									...
									{
										echo '<img src="/rets_photos/image-' . $listing->{FIELDNAME_RESIDENTIAL_LISTING_DATABASE_ID} . '-3.jpg" width="50" />';
									}
									?>
								</td>
								<td>
									...
									{
										echo '<img src="/rets_photos/image-' . $listing->{FIELDNAME_RESIDENTIAL_LISTING_DATABASE_ID} . '-4.jpg" width="50" />';
									}
									?>
								</td>
							</tr>
...
if ($listing->{FIELDNAME_LISTING_SUB_TYPE} == 'Condo' || $listing->{FIELDNAME_LISTING_SUB_TYPE} == 'Townhouse') {
	...
	echo "<span class=\"listing_subdivision_name\">" . ucwords(strtolower($listing->{FIELDNAME_RESIDENTIAL_ADDRESS_NUM} . " " . $listing->{FIELDNAME_RESIDENTIAL_STREET_NAME} . " " . $listing->{FIELDNAME_RESIDENTIAL_STREET_SUFFIX})) . "<br /></span>";
}
